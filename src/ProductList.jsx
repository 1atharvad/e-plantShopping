            ...
            {plantsArray.map((plantCategory, index) => (
                <div key={index}>
                    <div className='plantname_heading'>
                        <h1 className='plant_heading'>{plantCategory.category}</h1>
                    </div>
                    <div className='product-list'>
                        {plantCategory.plants.map((plant, plantIndex) => {
                            const itemInCart = cart.find(items => items.name === plant.name) === undefined;
                            return (
                                <div className="product-card" key={plantIndex}>
                                    <div className="product-title">{plant.name}</div>
                                    <img className="product-image" src={plant.image} alt={plant.name} />
                                    <div className='product-price'>{plant.cost}</div>
                                    <div className='product-description'><em>{plant.description}</em></div>
                                    <button className={`product-button${itemInCart ? '' : ' added-to-cart'}`} onClick={() => handleAddToCart(plant)} disabled={!itemInCart}>{itemInCart ? 'Add to Cart' : 'Added to Cart'}</button>
                                </div>
                            );
                        })}
                    </div>
